...
export default function Form(){
    ...
    if(!newGame.name){errors.name="Missing add the name"}

    if (!newGame.release_date){errors.release_date = "Missing add game release date"}
     else if(!/^\d{4}([-/.])(0?[1-9]|1[1-2])\1(3[01]|[12][0-9]|0?[1-9])$/.test(newGame.release_date))
    {errors.release_date="You must enter a valid format"}

    if(!newGame.rating) {errors.rating = " Enter the rating of the game "}
    else if (!/^[0-5]([.][0-9]{1,2})?/.test(Number(newGame.rating)))
    {errors.rating="The rating must be between 1 and 5"}

    if(!newGame.description) {errors.description = "Missing add description"} 

    if(!newGame.image){errors.image= "Missing add the URL of the image"}
    ...
    if(e.target.name=== "plataform") {newGame["plataform"].push(e.target.value);}

    else if(e.target.name === "genre") {newGame["genre"].push(e.target.value);}
